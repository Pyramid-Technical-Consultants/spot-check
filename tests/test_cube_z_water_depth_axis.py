...
def _interpolate_tick_depth_mm(
    ...
    """Depth (mm) implied by tick strings along scene zmin (index 0) → zmax."""
    z_lo = float(spec.zmin_scene)
    z_hi = float(spec.zmax_scene)
    if z_hi == z_lo:
        return float(tick_depth_mm[0])
    frac = (float(z_scene) - z_lo) / (z_hi - z_lo)
...
def _assert_cube_z_depth_mapping(actor, spec: CubeZAxisSpec) -> None:
    ...
    assert deep > shallow, "z_axis_range[0] must be deep mm, not ascending shallow-first"

    zr = actor.z_axis_range
    assert float(zr[0]) == pytest.approx(deep)
    assert float(zr[1]) == pytest.approx(shallow)
...
def test_regression_apply_does_not_flip_to_ascending_shallow_first_z_axis_range() -> None:
    """Regression: ascending range + ascending labels swaps deep/shallow tick text (~50 vs ~130)."""
    ...
    try:
        ...
        # Broken pairing (historical bug): shallow→deep ascending range and label strings.
...
def test_t0g10_plan_spots_and_cube_ticks_use_csda_mm_not_mev() -> None:
    ...
    z = nominal_mev_to_plot_z(e_all, use_proton_water_depth_mm=True)
    spec = cube_z_axis_spec(z, use_proton_water_depth_mm=True, tick_mm=5.0)
...
def test_t0g40_deepest_layer_same_as_t0g10_shallowest_differs() -> None:
    ...
    z = nominal_mev_to_plot_z(np.array([s[2] for s in planned]), use_proton_water_depth_mm=True)
    spec = cube_z_axis_spec(z, use_proton_water_depth_mm=True, tick_mm=5.0)
    deep, shallow = _cube_z_depth_label_endpoints(spec)
