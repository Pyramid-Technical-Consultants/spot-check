...
def test_label_at_scene_z_from_spec() -> None:
    ...
    assert label_at_scene_z(0.0, spec) == pytest.approx(0.0, abs=0.51)
    assert label_at_scene_z(10.0, spec) == pytest.approx(10.0, abs=0.51)


def test_cube_z_axis_spec_labels_match_scene_z_order() -> None:
    """Tick index 0 is at scene zmin — labels must follow scene Z, not max/min energy."""
    z = np.linspace(0.0, 10.0, 11)
